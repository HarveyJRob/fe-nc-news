...
const ArticleCard = ({ article, articlesList, setArticlesList }) => {
  ...
  return (
    <li className="flex-item">
      ...
      <p>
        <FontAwesomeIcon className="material-icons md-light md-24" icon={faThumbsUp} /> {article.votes}{" "}
        <FontAwesomeIcon className="material-icons md-light md-24" icon={faThumbsDown} /> |{" "}
        ...
        {loggedInUser === article.author && (
          <button onClick={() => handleDelete(article.article_id)}>
            <FontAwesomeIcon className="material-icons md-light md-24" icon={faTrashAlt} />
          </button>
        )}{" "}
        <FontAwesomeIcon className="material-icons md-light md-24" icon={faEdit} />{" "}
      </p>
    </li>
  );
};
